...
def main(args):
    ...
    for year in [2022,2023,2024]:
        ...
        for node_type in node_types:
            node_data[node_mapping[node_type]] = []
            ...
            for date_idx, date in enumerate(tqdm(dates, desc=f"Building {node_type} nodes feature")):
                ...
                try:
                    ...
                    daily_df = pd.concat([pd.read_csv(f) for f in glob.glob(file_path)])
            ...
            node_data[node_mapping[node_type]] = torch.tensor(feature_matrix, dtype=torch.float32)
            node_data[f'{node_mapping[node_type]}_latitudes'] = torch.tensor(node_info_df['latitude'].values, dtype=torch.float32)
            node_data[f'{node_mapping[node_type]}_longitudes'] = torch.tensor(node_info_df['longitude'].values, dtype=torch.float32)
        ...
        for node_type in node_types:
            ...
            node_data[f'healpix_feature_{node_mapping[node_type]}'] = healpix_node_feature
        ...
        for node_type in node_types:
            latitudes = node_data[f'{node_mapping[node_type]}_latitudes'].numpy()
            longitudes = node_data[f'{node_mapping[node_type]}_longitudes'].numpy()
            healpix_latitudes = node_data['healpix_latitudes'].numpy()
            healpix_longitudes = node_data['healpix_longitudes'].numpy()
            
            distance_cache[node_mapping[node_type]] = haversine(longitudes, latitudes, healpix_longitudes, healpix_latitudes)
        distance_cache['healpix'] = haversine(healpix_longitudes, healpix_latitudes, healpix_longitudes, healpix_latitudes)
        
        for node_type in node_types:
            for t in range(len(dates)):
                edges[(node_mapping[node_type], f't{t}_to_healpix', 'healpix')] = ([], [])
                distances[(node_mapping[node_type], f't{t}_to_healpix', 'healpix')] = []
            for t in range(len(dates)):
                edges[('healpix', f't{t}_to_{node_mapping[node_type]}', node_mapping[node_type])] = ([], [])
                distances[('healpix', f't{t}_to_{node_mapping[node_type]}', node_mapping[node_type])] = []

        edges[('healpix', 'healpix_message', 'healpix')] = ([], [])
        distances[('healpix', 'healpix_message', 'healpix')] = []

        k = args.number_of_nearest_neighbour
        nearest_nodes_indices = np.argsort(distance_cache['healpix'], axis=1)[:, 0:k+1] 
        src_nodes = np.repeat(np.arange(num_nodes_dict['healpix']), k+1)
        ...
        for node_type in node_types:
            all_mask[node_mapping[node_type]]=[]
             # Move distance_cache to GPU
            distance_cache[node_mapping[node_type]] = torch.tensor(distance_cache[node_mapping[node_type]], dtype=torch.float32).to('cuda') 
            for t in tqdm(range(len(dates)-1),desc=f"Building {node_mapping[node_type]} edge feature"):
                input_node_features = node_data[node_mapping[node_type]][:, t].to('cuda')
                output_node_features = node_data[node_mapping[node_type]][:, t+1].to('cuda')
                
                mask = ~torch.isnan(input_node_features) & ~torch.isnan(output_node_features) 
                all_mask[node_mapping[node_type]].append(mask.cpu())
                num_nodes_dict[f'{node_mapping[node_type]}_{t}'] = torch.sum(mask.cpu())

                valid_nodes = torch.nonzero(mask).squeeze()
                node_to_healpix_distances = distance_cache[node_mapping[node_type]][mask]

                nearest_nodes_indices = torch.argsort(node_to_healpix_distances, axis=0)[:args.number_of_nearest_neighbour, :]
                nearest_nodes = nearest_nodes_indices.T

                edge_src_nodes = nearest_nodes.reshape(-1)
                edge_dst_nodes = torch.repeat_interleave(torch.arange(nearest_nodes_indices.shape[1], device='cuda'), args.number_of_nearest_neighbour)

                nearest_healpix_indices = torch.argsort(node_to_healpix_distances, axis=1)[:, :args.number_of_nearest_neighbour]
                edge_src_healpix = nearest_healpix_indices.flatten()
                edge_dst_healpix = torch.repeat_interleave(torch.arange(nearest_healpix_indices.shape[0], device='cuda'), args.number_of_nearest_neighbour)

                edges[(node_mapping[node_type], f't{t}_to_healpix', 'healpix')][0].extend(edge_src_nodes.flatten().cpu().numpy())
                edges[(node_mapping[node_type], f't{t}_to_healpix', 'healpix')][1].extend(edge_dst_nodes.flatten().cpu().numpy())
                distances[(node_mapping[node_type], f't{t}_to_healpix', 'healpix')].extend(node_to_healpix_distances[nearest_nodes.flatten(),edge_dst_nodes.flatten()].cpu().numpy())
                ...
                edges[('healpix', f't{t+1}_to_{node_mapping[node_type]}', node_mapping[node_type])][0].extend(edge_src_healpix.cpu().numpy())
                edges[('healpix', f't{t+1}_to_{node_mapping[node_type]}', node_mapping[node_type])][1].extend(edge_dst_healpix.cpu().numpy())
                distances[('healpix', f't{t+1}_to_{node_mapping[node_type]}', node_mapping[node_type])].extend(node_to_healpix_distances[edge_dst_healpix,edge_src_healpix ].cpu().numpy())
        ...
        for i in tqdm(range(len(dates) - args.input_day-args.output_day+1),desc=f"Building each graph"):
            ...
            output_file = os.path.join(args.base_path,f"Realtime_regular_2022_2024", f"input_day_{args.input_day}_output_day_{args.output_day}_{date_str}.bin")
            ...
            for node_type in node_types:
                for t in range(input_time_steps):
                    edges_graph[(node_mapping[node_type], f't{t}_to_healpix', 'healpix')] = edges[(node_mapping[node_type], f't{t+i}_to_healpix', 'healpix')]
                    distances_graph[(node_mapping[node_type], f't{t}_to_healpix', 'healpix')] = distances[(node_mapping[node_type], f't{t+i}_to_healpix', 'healpix')]
                for t in range(input_time_steps,input_time_steps+output_time_steps):
                    edges_graph[('healpix', f't{t}_to_{node_mapping[node_type]}', node_mapping[node_type])] = edges[('healpix', f't{t+i}_to_{node_mapping[node_type]}', node_mapping[node_type])]
                    distances_graph[('healpix', f't{t}_to_{node_mapping[node_type]}', node_mapping[node_type])] = distances[('healpix', f't{t+i}_to_{node_mapping[node_type]}', node_mapping[node_type])]
            ...
            for node_type in node_types:
                split_num_nodes_dict[node_mapping[node_type]]=num_nodes_dict[f'{node_mapping[node_type]}_{i}']
            ...
            for node_type in node_types:
                for t in range(input_time_steps+output_time_steps):
                    hetero_graph.nodes[node_mapping[node_type]].data[f't{t}'] = node_data[node_mapping[node_type]][:,t+i][all_mask[node_mapping[node_type]][i]].unsqueeze(1)
                    hetero_graph.nodes['healpix'].data[f'{node_mapping[node_type]}_t{t}'] = node_data[f'healpix_feature_{node_mapping[node_type]}'][:,t+i].unsqueeze(1)
                hetero_graph.nodes[node_mapping[node_type]].data['latitude'] = node_data[f'{node_mapping[node_type]}_latitudes'][all_mask[node_mapping[node_type]][i]].unsqueeze(1)
                hetero_graph.nodes[node_mapping[node_type]].data['longitude'] = node_data[f'{node_mapping[node_type]}_longitudes'][all_mask[node_mapping[node_type]][i]].unsqueeze(1)
            ...
            dgl.save_graphs(output_file, [hetero_graph],{node_mapping[node_type]:all_mask[node_mapping[node_type]][i] for node_type in node_types})
