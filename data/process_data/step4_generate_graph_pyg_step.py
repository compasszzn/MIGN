...
def main(args):
    ...
    for variable in variables:
        ...
        for i in tqdm(range(len(csv_files) - args.input_day - args.output_day + 1), desc=f"Processing {variable}"):
            daily_df_output = pd.read_csv(csv_files[i+args.input_day], dtype={'station_id': str})
            ...
            mask = ~torch.isnan(selected_output_values)
            ...
            for k in range(args.input_day):
                ...
                mask = ~torch.isnan(selected_input_values) & mask 
