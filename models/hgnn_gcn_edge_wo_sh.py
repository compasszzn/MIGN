...
class Graph_Encoder(nn.Module):
    def __init__(self, model_args,data_args):
        ...
        if self.model_args['sh_before']:
            self.position_embedding_src ={var: torch.load(f"{self.data_args['embedding_dir']}/{var}_embeddings.pt").to('cuda') for var in self.data_args['vars']}
            self.position_embedding_dst = torch.load(f"{self.data_args['embedding_dir']}/healpix_embeddings.pt").to('cuda')
    ...
    def forward(self, g,timestamp,sh_embedding=None,position_embedding_feature=None,position_embedding_healpix=None):
        # for node_type, linear in self.node_type_to_healpix.items():
...
class MLP_Processor(nn.Module):
    ...
    def forward(self,x):
        ...
        attn_x = x.permute(0,2,1,3).reshape(-1,x.size(1),x.size(3))
        ...
        x = x + attn_output
        x = x.permute(0,3,1,2)
        # x=x.reshape(x.shape[0],x.shape[1],-1,self.hidden_size)
        return x
...
class Graph_Decoder(nn.Module):
    def __init__(self, model_args,data_args):
        ...
        if self.model_args['sh_after']:
            self.position_embedding_src = torch.load(f"{self.data_args['embedding_dir']}/healpix_embeddings.pt").to('cuda')
            self.position_embedding_dst = {var: torch.load(f"{self.data_args['embedding_dir']}/{var}_embeddings.pt").to('cuda') for var in self.data_args['vars']}
        if self.model_args['sh_after']:
            self.linears = nn.ModuleDict({node_type: nn.Linear(1+9,64) for node_type in self.data_args['vars']})
        else:
            self.linears = nn.ModuleDict({node_type: nn.Linear(1,64) for node_type in self.data_args['vars']})
        ...
        for idx, node_type in enumerate(self.data_args['vars']):
            for t in range(self.output_length):
                ...
                if self.model_args['sh_after']:
                    ...
                    space_2 = self.linears[node_type](g.nodes[node_type].data[f't{t}'].unsqueeze(0).unsqueeze(0).unsqueeze(0))#torch.Size([1, 1, 1, 20531, 64])
