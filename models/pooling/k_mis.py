...
class KMISSelect(Select):
    ...
    def _apply_heuristic(self, x: Tensor, adj: SparseTensor) -> Tensor:
        ...
        for _ in range(self.k):
            k_sums = scatter(k_sums[row], col, dim_size=adj.size(0),
                             reduce='add')###(k_sums torch.Size([33191]))
    ...
    def forward(
        ...
        if self.force_undirected:
            if isinstance(edge_index, SparseTensor):
                edge_index, edge_attr = connectivity_to_edge_index(edge_index)## sparse to edge index torch.Size([469304])
            edge_index, edge_attr = to_undirected(edge_index, edge_attr,
                                                  num_nodes, reduce='max')## torch.Size([2, 567928])
        adj = connectivity_to_adj_t(edge_index, edge_attr, num_nodes=num_nodes)## edge index to sparse ([445223],undirected)
        score = self._scorer(x, adj, batch=batch, num_nodes=num_nodes)## without x default score all [1]
        updated_score = self._apply_heuristic(score, adj)## 相当于每个点的分数都是1，通过messge passing，每个点得到一个不同的分数
        perm = torch.argsort(updated_score.view(-1), 0, descending=True)

        mis, cluster = maximal_independent_set_cluster(adj, self.k, perm)#perm torch.Size([42664])#all torch.Size([42664])
        mis = mis.nonzero().view(-1)#mis [6024]
