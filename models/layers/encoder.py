...
class Encoder(nn.Module):
    ...
    def __init__(self,
                 ...
                 activation: str = "linear",feature=None):
        super().__init__()
        ...
        if self.feature is not None:
            self.position_lin = nn.Linear(9, hidden_size)
    ...
    def forward(self, x: Tensor, mask: Optional[Tensor] = None,
                u: Optional[Tensor] = None,sh_feature = None):
        ...
        if self.feature is not None:
            position_emb = sh_feature
            position_emb = position_emb.view(self.compute_shape(position_emb, out))
            out = out + self.position_lin(position_emb)
        if mask is not None:
            if mask.ndim != x.ndim:
