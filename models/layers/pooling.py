...
from models.pooling import KMISPooling
from models.pooling.topk import TOPKPooling
from models.pooling import connectivity_to_adj_t, Pooling, SelectOutput
from models.pooling.aggr_pool import ReductionType, ConnectionType, LiftMatrixType
...
class HierarchicalPooling(nn.Module):
    ...
    def compute_coarsened_graphs(self, edge_index: Tensor,
                                 ...
                                 feature = None):
        out = []
        adj = connectivity_to_adj_t(edge_index, edge_attr, num_nodes=num_nodes)## get sparse tensor adj.coo()
        for _ in range(self.n_layers):
            # select
            s = self.pool.select(adj,x=feature, num_nodes=num_nodes)
            # store initial graph
            s.adj = adj
            # lift
            ...
            # connect
            adj, _ = self.pool.connect(adj, s=s)#adj [365101,33191] ##s []
    ...
    def recursive_lifting_pooling(self, x: Tensor, graphs: List[SelectOutput]) \
        ...
        for convs, graph in zip(self.mps, graphs):
            ...
            # 1st message-passing
            x = conv_h(x, graph.adj)
            # pooling x([1, 1, 33191, 64])
            x = self.pool.reduce(x, s=graph)
            # after pooling torch.Size([1, 1, 4562, 64])
            pooled.append(x)
        ...
        for i in range(1, len(graphs) + 1):
            ...
            out.append(x_out)#torch.Size([1, 1, 33191, 64])
    ...
    def forward(self, x: Tensor, edge_index: Tensor,
                edge_attr: Optional[Tensor] = None,
                cached: bool = False,
                feature = None) -> Tuple[Tensor, List[Tensor]]:
        # x: [..., node, features]
        num_nodes = x.size(-2) # 42664
        graphs = self.get_coarsened_graphs(edge_index, edge_attr, num_nodes,
                                           cached=cached,feature=feature)### pooliong
        ...
        #out [torch.Size([1, 1, 33191, 64])]  x_out [torch.Size([1, 1, 33191, 64])]
        out += x_out
        pooled += x_pooled
        #pooled [torch.Size([1, 1, 33191, 64])] x_pooled[torch.Size([1, 1, 4562, 64])]
...
class HierPoolFactory(HierarchicalPooling):
    ...
    def __init__(self, input_size: int,
        ...
        #  POOLING LAYER  #####################################################
        pool = KMISPooling(in_channels=9,reduce=reduce_op,
                        connect=connect_op,
                        scorer="constant",
                        remove_self_loops=True,
                        force_undirected=True)
        # elif pooling == 'topk':
        #     pool = TOPKPooling(in_channels=9,reduce=reduce_op,
        #                 connect=connect_op,
        #                 scorer="constant",
        #                 remove_self_loops=True,
        #                 force_undirected=True)
